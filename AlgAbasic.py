...
#I use this for all my basic and enhanced algorithms. It is just to display a matrix. This could be the distance matrix or
#the pheromone matrix in my ACO algorithms.
...
#randomly generate a population of chromosomes. Chromosomes having random routes with the first run. 
...
#Finding the fitness os the chromosomes (length of the tours)
...
#Finding a probability metric for the roulette wheel selection process.
def fitness_probability(fitness_score):
    total = sum(fitness_score)
    initial_values = []
    probability_values = []
    
    for i in range(len(fitness_score)):
        initial_values.append(total/fitness_score[i])
        
    total = sum(initial_values)
    
    for i in range(len(initial_values)):
        probability_values.append(initial_values[i]/total)
        (probability_values[i]).as_integer_ratio()

    return probability_values


#Roulette Wheel Selection: will spin a "metaphorical wheel". Essentially there is a random probability generated.
#if the probability of the chromosome being selected is higher than this random probability, then it will be placed in the selection list.
...
#crossover consists of randomly matching 1 part of 2 chromosomes to create children.
...
#adds children from crossover into the population
...
#will randomly select a chromosome and give it a mutation. This could increase the chances of mutating to a fitter chromosome.
...
def geneticAlgorithm():
    ...
    while time.time() - start_time < 59 and count < 1000:
        count += 1
        i += 1
        parents = RouletteWheelSelection(population, fitness_Scores)
